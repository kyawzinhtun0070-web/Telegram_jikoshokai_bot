"""
Yay Zat Bot — Simple Polling Version (100% Clean)
"""
import telebot
import sqlite3
import threading
...
ADMIN_ID = 6131831207
HEART_STICKER = "CAACAgIAAxkBAAEBmjFnQ_example"
...
def db_save(uid, data):
    try:
        ...
        cols = ','.join(UF)
        ph = ','.join(['?'] * len(UF))
        vals = [data.get(f) for f in UF]        upd = ','.join([f"{f}=excluded.{f}" for f in UF])
        query = f"INSERT INTO users (user_id,{cols}) VALUES(?,{ph}) ON CONFLICT(user_id) DO UPDATE SET {upd}"
        xq(query, [uid] + vals)
    except:
        pass
...
def db_ids():
    try:
        result = xr('SELECT user_id FROM users')
        if result:
...
def db_seen_add(u, s):
    try:
        xq('INSERT OR IGNORE INTO seen VALUES(?,?)', (u, s))    except:
        pass
...
def db_seen_clear(uid):
    try:
        xq('DELETE FROM seen WHERE user_id=?', (uid,))
    except:
        pass
...
def fmt(tp, title='👤 *ပရိုဖိုင်*'):
    ...
    return (
        ...
        f"{bio}\n"
        f"⚧:{sf(tp,'gender')}\n"
        f"🔍:{sf(tp,'looking_gender')}/{sf(tp,'looking_zodiac','Any')}"
    )

def send_safe(uid, photo, caption, markup):
    if photo:        try:
            bot.send_photo(uid, photo, caption=caption, reply_markup=markup, parse_mode="Markdown", timeout=30)
            return
        except:
            pass
...
def _sk(msg):
    return msg.text and msg.text.strip() == '/skip'
...
def r_age(m):
    if _is_start(m):
        return
    uid = m.chat.id
    if not _sk(m) and m.text and m.text.strip().isdigit():
        _sv(uid, 'age', m)
...
def r_city(m):
    ...
    if not _sk(m):
        _sv(uid, 'city', m)
    bot.send_message(uid, "🎨 ဝါသနာ? (/skip)-", timeout=30)
    safe_next(m, r_hobby)
...
def r_job(m):
    ...
    if not _sk(m):
        _sv(uid, 'job', m)
    bot.send_message(uid, "📝 Bio? (/skip)-", parse_mode="Markdown", timeout=30)
    safe_next(m, r_bio)
...
def r_bio(m):
    ...
    bot.send_message(uid, "⚧ လိင်?", reply_markup=mk, timeout=30)
    safe_next(m, r_gender)
...
def r_gender(m):
    ...
    bot.send_message(uid, "💑 ရှာမည့်လိင်?", reply_markup=mk, timeout=30)
    safe_next(m, r_lgender)

def r_lgender(m):    if _is_start(m):
        return
    uid = m.chat.id
    if not _sk(m):
        _sv(uid, 'looking_gender', m)
    mk = ReplyKeyboardMarkup(one_time_keyboard=True)
    for z in ZODIACS:
        mk.add(z)
    mk.add('Any', '/skip')
    bot.send_message(uid, "🔮 ရှာမည့်ရာသီ?", reply_markup=mk, timeout=30)
    safe_next(m, r_lzodiac)
...
def r_lzodiac(m):
    ...
    uid = m.chat.id
    if not _sk(m):
        _sv(uid, 'looking_zodiac', m)
    bot.send_message(uid, "📸 ဓာတ်ပုံ? (/skip)-", parse_mode="Markdown", reply_markup=ReplyKeyboardRemove(), timeout=30)
    safe_next(m, r_photo)
...
def r_photo(m):
    ...
    if _sk(m):
        ...
        bot.send_message(uid, "⚠️ ဓာတ်ပုံ (သို့) /skip-", timeout=30)
        safe_next(m, r_photo)
        return
    data = _reg.pop(uid, {})
    db_save(uid, data)
    status = 'အသစ်' if is_new else 'ပြင်'
    bot.send_message(uid, f"✅ Profile {status}ပြီး!", reply_markup=kb(uid), timeout=30)
    if is_new:
        admin_msg(f"🆕 User `{uid}` — {sf(data,'name')}")
...
def cmd_start(m):
    ...
    _reg.pop(uid, None)
    try:
        bot.clear_step_handler_by_chat_id(uid)    except:
        pass
    if db_get(uid):
        bot.send_message(uid, "✨ *ကြိုဆိုပါတယ်!*", parse_mode="Markdown", reply_markup=kb(uid), timeout=30)
        return
    begin_reg(uid, m)
...
def find_match(m):
    ...
    me = db_get(uid)
    if not me:
        bot.send_message(uid, "/start နှိပ်ပါ", reply_markup=kb(uid), timeout=30)
        return
    ...
    pool = []
    for u in db_all():
        if u['user_id'] in excl:
            continue
        if lg and lg not in ('Both', 'Any', ''):
            if (u.get('gender') or '').strip() != lg:
                continue
        pool.append(u)
    if not pool:
        if seen:
            db_seen_clear(uid)
            bot.send_message(uid, "🔄 ပြန်စ...", timeout=30)
            find_match(m)
            return
        bot.send_message(uid, "😔 မတွေ့", reply_markup=kb(uid), timeout=30)
        return
    if lz and lz not in ('Any', ''):
        pool = sorted(pool, key=lambda x: 0 if (x.get('zodiac') or '') == lz else 1)
    tgt = pool[0]
    tid = tgt['user_id']
    db_seen_add(uid, tid)
...
def show_profile(m):
    ...
    tp = db_get(uid)
    if not tp:
        bot.send_message(uid, "Profile မရှိ", reply_markup=kb(uid), timeout=30)        return
    mk = InlineKeyboardMarkup()
    fields = ['name','age','zodiac','city','hobby','job','bio','photo','gender','looking_gender']
    for f in fields:
        mk.row(InlineKeyboardButton(f"📝 {f}", callback_data=f"e_{f}"))
    mk.row(
        InlineKeyboardButton("🔄 ပြန်လုပ်", callback_data="do_reset"),
...
def menu_router(m):
    ...
    if m.text == "🔍 ရှာမည်":
        find_match(m)
    elif m.text == "👤 ကိုယ့်ပရိုဖိုင်":
        show_profile(m)
    else:
        bot.send_message(uid, "🚧", reply_markup=kb(uid), timeout=30)

EDIT_MAP = {
    'name':'နာမည်', 'age':'အသက်', 'zodiac':'ရာသီ', 'city':'မြို့',
    'hobby':'ဝါသနာ', 'job':'အလုပ်', 'bio':'Bio', 'photo':'ဓာတ်ပုံ',
    'gender':'လိင်', 'looking_gender':'ရှာဖွေမည့်လိင်'
}

def save_edit(m, field):
    uid = m.chat.id
    if field == 'photo' and m.content_type != 'photo':
        bot.send_message(uid, "⚠️ ဓာတ်ပုံသာ ပေးပို့ပါ", timeout=30)
        return
    val = None
    if field == 'photo':
        ...
        val = m.text.strip()
    if val:
        db_update(uid, field, val)
        bot.send_message(uid, "✅ ပြင်ပြီး", reply_markup=kb(uid), timeout=30)
    else:
        bot.send_message(uid, "✅ မပြောင်းဘဲ ထိန်း", reply_markup=kb(uid), timeout=30)

@bot.callback_query_handler(func=lambda c: True)
def on_cb(call):    uid = call.message.chat.id
    d = call.data
    try:
        if d.startswith("like_"):
            ...
            try:
                ...
                pass
            try:
                bot.send_sticker(uid, HEART_STICKER, timeout=30)
            except:
                bot.send_message(uid, "❤️", timeout=30)
            me = db_get(uid) or {}
            mk = InlineKeyboardMarkup()
            mk.row(
                InlineKeyboardButton("✅", callback_data=f"accept_{uid}"),
            ...
            for me, p in [(uid, liker), (liker, uid)]:
                pd = db_get(p)
                bot.send_message(me, f"💖 Match! [နှိပ်](tg://user?id={p})", parse_mode="Markdown", reply_markup=kb(me), timeout=30)
        elif d == "decline":
            try:
                bot.delete_message(uid, call.message.message_id)
            except:
            ...
            try:
                ...
                pass
            begin_reg(uid, call.message, prefill=db_get(uid))
        elif d == "do_delete":
            ...
            db_delete(uid)
            _reg.pop(uid, None)
            bot.send_message(uid, "🗑 ဖျက်ပြီး", reply_markup=ReplyKeyboardRemove(), timeout=30)
            ...
            try:
                ...
                pass
            if f == 'photo':
                msg = bot.send_message(uid, "📸 ဓာတ်ပုံ ပေးပို့ပါ", reply_markup=ReplyKeyboardRemove(), timeout=30)
                safe_next(msg, save_edit, 'photo')
            else:
                label = EDIT_MAP.get(f, f)
                msg = bot.send_message(uid, f"📝 *{label}* ရိုက်ပါ", parse_mode="Markdown", reply_markup=ReplyKeyboardRemove(), timeout=30)
                safe_next(msg, save_edit, f)
        bot.answer_callback_query(call.id, timeout=30)
    except:
        pass

# ═════ POLLING LOOP ═════
print(f"✅ Yay Zat Bot Started [{datetime.now()}]")
