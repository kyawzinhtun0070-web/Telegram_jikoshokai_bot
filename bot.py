"""
Yay Zat Bot — Koyeb Ready (Clean Version)
"""
import os, telebot, sqlite3, threading, time, requests
...
def xr(sql, p=()):
    with _lk:
        for _ in range(3):
            try:
                return _db.execute(sql, p)            except:
                open_db()
                time.sleep(0.3)
        return None
...
def db_update(uid, f, v):
    if f in UF:
        try: xq(f"UPDATE users SET {f}=? WHERE user_id=?", (v, uid))
        except: pass
...
def fmt(tp, t='👤 *ပရိုဖိုင်*'):
    bio = f"\n📝 Bio:{sf(tp,'bio')}" if tp and tp.get('bio') else ''
    return f"{t}\n\n📛:{sf(tp,'name')}\n🎂:{sf(tp,'age')}\n🔮:{sf(tp,'zodiac')}\n📍:{sf(tp,'city')}\n🎨:{sf(tp,'hobby')}\n💼:{sf(tp,'job')}{bio}\n⚧:{sf(tp,'gender')}\n🔍:{sf(tp,'looking_gender')}/{sf(tp,'looking_zodiac','Any')}"

def send_safe(uid, photo, cap, mk):
    if photo:
        try:
            bot.send_photo(uid, photo, caption=cap, reply_markup=mk, parse_mode="Markdown", timeout=30)
            return
        except: pass
...
def r_zodiac(m):
    if _is_start(m): return
    uid = m.chat.id
    if not _sk(m): _sv(uid, 'zodiac', m)    bot.send_message(uid, "📍 မြို့? (/skip)-", reply_markup=ReplyKeyboardRemove(), timeout=30)
    safe_next(m, r_city)
...
def cmd_start(m):
    ...
    if db_get(uid):
        bot.send_message(uid, "✨ ကြိုဆို!", parse_mode="Markdown", reply_markup=kb(uid), timeout=30)
        return
    begin_reg(uid, m)
...
def find_match(m):
    ...
    mk.row(InlineKeyboardButton("❤️", callback_data=f"like_{tid}"), InlineKeyboardButton("👎", callback_data=f"nope_{tid}"))
...
def on_cb(call):
    ...
    try:
        if d.startswith("like_"):
            ...
            bot.send_message(uid, "🚧 Edit", reply_markup=kb(uid), timeout=30)
